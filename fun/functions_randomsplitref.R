...
# Set of functions to use on exported Rayyan csv file to randomly subset references, and to split reference list.
# Output files from functions are csv files that can be read into Rayyan.
...
### IMPORTANT: Before running tests make sure to run all functions

# Set root data directory and paths where to save files
datdir <- rprojroot::find_rstudio_root_file("splitRef")
filePath <- file.path(datdir, "articles.csv")


#### getpilotref function 
getpilotref(filePath, n=15, fileName="pilot_test1")
